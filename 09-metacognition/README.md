[![多 Agent 系统 Design](./images/lesson-9-thumbnail.png)](https://youtu.be/His9R6gw6Ec?si=3_RMb8VprNvdLRhX)
...
Welcome to the lesson on metacognition in AI Agent! This chapter is designed for beginners who are curious about how AI Agent can think about their own thinking processes. By the end of this lesson, you'll understand key concepts and be equipped with practical examples to apply metacognition in AI Agent design.

## 学习目标

完成本节课后, you'll be able to:

1. Understand the implications of reasoning loops in Agent definitions.
2. Use planning and evaluation techniques to help self-correcting Agent.
3. Create your own Agent capable of manipulating code to accomplish tasks.

## 简介 to 元认知

元认知 refers to the higher-order cognitive processes that involve thinking about one’s own thinking. For AI Agent, this means being able to evaluate and adjust their actions based on self-awareness and past experiences. 元认知, or "thinking about thinking," is an important concept in the development of Agentic AI systems. It involves AI systems being aware of their own internal processes and being able to monitor, regulate, and adapt their behavior accordingly. Much like we do when we read the room or look at a problem. This self-awareness can help AI systems make better decisions, identify errors, and improve their performance over time- again linking back to the Turing test and the debate over whether AI is going to take over.
...
元认知, or "thinking about thinking," is a higher-order cognitive process that involves self-awareness and self-regulation of one's cognitive processes. In the realm of AI, metacognition empowers Agent to evaluate and adapt their strategies and actions, leading to improved problem-solving and decision-making capabilities. By understanding metacognition, you can design AI Agent that are not only more intelligent but also more adaptable and efficient. In true metacognition, you’d see the AI explicitly reasoning about its own reasoning.
...
- Diagnosing 模式s like, “Whenever I see the user mention ‘too crowded,’ I should not only remove certain attractions but also reflect that my method of picking ‘top attractions’ is flawed if I always rank by popularity.”

### Importance of 元认知 in AI Agent

元认知 plays a crucial role in AI Agent design for several reasons:

![Importance of 元认知](./images/importance-of-metacognition.png)

- Self-Reflection: Agent can assess their own performance and identify areas for improvement.
- Adaptability: Agent can modify their strategies based on past experiences and changing environments.
- Error Correction: Agent can detect and correct errors autonomously, leading to more accurate outcomes.
- Resource Management: Agent can optimize the use of resources, such as time and computational power, by planning and evaluating their actions.
...
- 工具s: The capabilities and functions that the Agent can perform.
...
### Example: 元认知 in a Travel Agent Service

Imagine you're designing a travel Agent service powered by AI. This Agent, "Travel Agent," assists users with planning their vacations. To incorporate metacognition, Travel Agent needs to evaluate and adjust its actions based on self-awareness and past experiences. Here's how metacognition could play a role:
...
Here's a simplified example of how Travel Agent code might look when incorporating metacognition:
...
- **Self-Reflection**: Agent can analyze their performance and identify areas for improvement.
- **Adaptability**: Agent can modify strategies based on feedback and changing conditions.
- **Error Correction**: Agent can autonomously detect and correct mistakes.
- **Resource Management**: Agent can optimize resource usage, such as time and computational power.
...
Firstly let's start by understanding the difference between RAG 工具 and Pre-emptive Context Load
...
The Corrective RAG approach focuses on using RAG techniques to correct errors and improve the accuracy of AI Agent. This involves:

1. **Prompting Technique**: Using specific prompts to guide the Agent in retrieving relevant information.
2. **工具**: Implementing algorithms and mechanisms that enable the Agent to evaluate the relevance of the retrieved information and generate accurate responses.
...
2. **工具**: Using natural language processing and machine learning algorithms to rank and filter search results.
...
- **工具:** Implementing algorithms and mechanisms that enable the Agent to evaluate the relevance of the retrieved information and generate accurate responses.
...
5. **Corrective RAG Process**
   ...
   - **工具**: Travel Agent uses algorithms to rank and filter new search results, emphasizing the relevance based on user feedback.
     - Example:
...
### Taking Advantage of 大型语言模型 for Re-ranking and Scoring

Large Language Models (大型语言模型s) can be used for re-ranking and scoring by evaluating the relevance and quality of retrieved documents or generated responses. Here's how it works:

**Retrieval:** The initial retrieval step fetches a set of candidate documents or responses based on the query.

**Re-ranking:** The 大型语言模型 evaluates these candidates and re-ranks them based on their relevance and quality. This step ensures that the most relevant and high-quality information is presented first.

**Scoring:** The 大型语言模型 assigns scores to each candidate, reflecting their relevance and quality. This helps in selecting the best response or document for the user.

By leveraging 大型语言模型s for re-ranking and scoring, the system can provide more accurate and contextually relevant information, improving the overall user experience.

Here's an example of how a travel Agent might use a Large Language Model (大型语言模型) for re-ranking and scoring travel destinations based on user preferences in Python:

#### Scenario - Travel based on Preferences

A travel Agent wants to recommend the best travel destinations to a client based on their preferences. The 大型语言模型 will help re-rank and score the destinations to ensure the most relevant options are presented.

#### Steps:

1. Collect user preferences.
2. Retrieve a list of potential travel destinations.
3. Use the 大型语言模型 to re-rank and score the destinations based on user preferences.
...
By leveraging the 大型语言模型 for re-ranking and scoring, the travel Agent can provide more personalized and relevant travel recommendations to clients, enhancing their overall experience.

### RAG: Prompting Technique vs 工具

Retrieval-Augmented Generation (RAG) can be both a prompting technique and a 工具 in the development of AI Agent. Understanding the distinction between the two can help you leverage RAG more effectively in your projects.
...
- As a 工具, RAG is an integrated system that automates the retrieval and generation process, making it easier for developers to implement complex AI functionalities without manually crafting prompts for each query.
...
2. **Automation**: The 工具 manages the entire process, from receiving user input to generating the final response, without requiring explicit prompts for each step.
...
- User Input: "I want to visit museums in Paris."
- RAG 工具: Automatically retrieves information about museums and generates a response.
- Generated Response: "Here are some top museums in Paris: Louvre Museum, Musée d'Orsay, and Centre Pompidou."

### Comparison

| Aspect                 | Prompting Technique                                        | 工具                                                  |
|------------------------|-------------------------------------------------------------|-------------------------------------------------------|
...
class Travel_Agent:
    ...
    def get_museums_in_paris(self):
        ...
        response = self.rag_工具.retrieve_and_generate(user_input)
        return response
...
Evaluating relevancy is a crucial aspect of AI Agent performance. It ensures that the information retrieved and generated by the Agent is appropriate, accurate, and useful to the user. Let's explore how to evaluate relevancy in AI Agent, including practical examples and techniques.
...
## 4. Generating Code as a 工具

Code generating Agent use AI models to write and execute code, solving complex problems and automating tasks.

### Code Generating Agent

Code generating Agent use generative AI models to write and execute code. These Agent can solve complex problems, automate tasks, and provide valuable insights by generating and running code in various programming languages.
...
1. **Task**: Analyze a dataset to identify trends and 模式s.
2. **Steps**:
   - Load the dataset into a data analysis 工具.
...
3. **Required Resources**: Access to the dataset, data analysis 工具s, and SQL capabilities.
...
SQL (Structured Query Language) is a powerful 工具 for interacting with databases. When used as part of a Retrieval-Augmented Generation (RAG) approach, SQL can retrieve relevant data from databases to inform and generate responses or actions in AI Agent. Let's explore how SQL can be used as a RAG technique in the context of Travel Agent.
...
By leveraging SQL as part of the Retrieval-Augmented Generation (RAG) technique, AI Agent like Travel Agent can dynamically retrieve and utilize relevant data to provide accurate and personalized recommendations.
...
元认知 is a powerful 工具 that can significantly enhance the capabilities of AI Agent. By incorporating metacognitive processes, you can design Agent that are more intelligent, adaptable, and efficient. Use the additional resources to further explore the fascinating world of metacognition in AI Agent.

### Got More Questions about the 元认知 Design 模式?
...
[多 Agent 系统 Design 模式](../08-multi-Agent/README.md)

## 下一课

[生产环境中的 AI Agent](../10-ai-Agent-production/README.md)
